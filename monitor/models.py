...
class MonitorRules(models.Model):
    ...
    item = models.ForeignKey(
        MonitorItems, on_delete=models.SET_NULL, verbose_name=_("Item"), null=True
    )
    item_threshold = models.FloatField(
        _("Threshold"), default=0, help_text="Float Value"
    )
...
def validate_time(value):
    if value < 1 or value > 24:
        raise ValidationError(_("Range Time between 1 and 24!"))
...
class OperationalTime(models.Model):
    ...
    def clean(self):
        ...
        if self.mon_end < self.mon_start:
            raise ValidationError(
                {"mon_start": _("Start Time is bigger than End Time!")}
            )
        if self.tue_end < self.tue_start:
            raise ValidationError(
                {"tue_start": _("Start Time is bigger than End Time!")}
            )
        if self.wed_end < self.wed_start:
            raise ValidationError(
                {"wed_start": _("Start Time is bigger than End Time!")}
            )
        if self.thu_end < self.thu_start:
            raise ValidationError(
                {"thu_start": _("Start Time is bigger than End Time!")}
            )
        if self.fri_end < self.fri_start:
            raise ValidationError(
                {"fri_start": _("Start Time is bigger than End Time!")}
            )
        if self.sat_end < self.sat_start:
            raise ValidationError(
                {"sat_start": _("Start Time is bigger than End Time!")}
            )
        if self.sun_end < self.sun_start:
            raise ValidationError(
                {"sun_start": _("Start Time is bigger than End Time!")}
            )

        if self.id is None:
            try:
                OperationalTime.objects.get(
                    network=self.network, organization=self.organization
                )
                raise ValidationError(
                    {"network": _("Network already has Operation Time")}
                )
    ...
    def get_operational_time(self):
        ...
        if self.is_tue:
            text.append(
                "{}: {} - {}".format(_("TUESDAY"), self.tue_start, self.tue_end)
            )
        if self.is_wed:
            text.append(
                "{}: {} - {}".format(_("WEDNESDAY"), self.tue_start, self.tue_end)
            )
        if self.is_thu:
            text.append(
                "{}: {} - {}".format(_("THURSDAY"), self.thu_start, self.thu_end)
            )
        if self.is_fri:
            text.append("{}: {} - {}".format(_("FRIDAY"), self.fri_start, self.fri_end))
        if self.is_sat:
            text.append(
                "{}: {} - {}".format(_("SATURDAY"), self.sat_start, self.sat_end)
            )
        if self.is_sun:
