...
class MonitorRulesAdmin(ModelAdmin):
    ...
    panels = [
        ...
        MultiFieldPanel(
            [FieldPanel("item"), FieldPanel("item_threshold")], heading=_("Item Detail")
        ),
    ]
    """
    ...
    def get_queryset(self, request):
        ...
        if not current_user.is_superuser:
            if current_user.organization.is_no_org:
                ...
                return MonitorRules.objects.filter(
...
class MemberProblemsAdmin(ModelAdmin):
    ...
    list_display = (
        "member_name_with_update_progress",
        "problem_duration_start",
        "get_parameters",
    )
    ...
    def get_queryset(self, request):
        current_user = get_current_user()
        problem_time = timezone.now() - timezone.timedelta(
            seconds=settings.MONITOR_DELAY
        )
        if not current_user.is_superuser:
            if current_user.organization.is_no_org:
                # return MemberProblems.objects.filter(member__user=current_user)
                return MemberProblems.unsolved.filter(
                    member__user=current_user, start_at__lt=problem_time
                ).order_by("start_at")
            else:
                return MemberProblems.unsolved.filter(
                    member__organization=current_user.organization,
                    start_at__lt=problem_time,
                ).order_by("start_at")
        else:
            return MemberProblems.unsolved.filter(start_at__lt=problem_time).order_by(
                "start_at"
            )
    ...
    def get_queryset(self, request):
        ...
        if not current_user.is_superuser:
            if current_user.organization.is_no_org:
                ...
                return MemberProblemsDone.objects.filter(
                    member__user=current_user, duration__gt=settings.MONITOR_DELAY
                ).order_by("-end_at")
            else:
                ...
                return MemberProblemsDone.objects.filter(
                    member__organization=current_user.organization,
                    duration__gt=settings.MONITOR_DELAY,
                ).order_by("-end_at")
        else:
    ...
    def get_queryset(self, request):
        ...
        if not current_user.is_superuser:
            if current_user.organization.is_no_org:
                ...
                return OperationalTime.objects.filter(
                    organization=current_user.organization
                )
        else:
...
class MonitorAdminGroup(ModelAdminGroup):
    ...
    items = (
        MonitorItemsAdmin,
        MonitorRulesAdmin,
        OperationalTimeAdmin,
        MemberProblemsHistoryAdmin,
    )
