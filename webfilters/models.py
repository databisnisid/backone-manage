...
class WebFilters(models.Model):
    ...
    domains = models.TextField(
        _("Black List"),
        help_text=_("List top domain to block"),
        default="atleastoneblacklistdomain.com",
    )
    domains_white = models.TextField(
        _("White List"),
        help_text=_("List top domain to allow"),
        default="atleastonewhitelistdomain.com",
    )
    is_default_block = models.BooleanField(
        _("Default Block All"),
        help_text=_("Check this for block all website as default"),
        default=False,
    )
    ...
    def clean(self):
        ...
        if not current_user.is_superuser:
            try:
                network = WebFiltersOrg.objects.get(
                ...
                if (
                    current_user.organization.features.is_webfilter
                    and not current_user.organization.features.is_webfilter_multinet
                ):
                    raise ValidationError(
                        _("Please setup Org<->Net correlation. Contact Administrator!")
                    )
                else:
                    pass
        ...
        if not current_user.is_superuser:
            if (
                ...
                try:
                    ...
                    return {"network": network.network, "is_waf": True}

                except ObjectDoesNotExist or MultipleObjectsReturned:
                    return {"organization": current_user.organization, "is_waf": True}
            else:
                return {"organization": current_user.organization, "is_waf": True}
        else:
            return {"is_waf": True}
